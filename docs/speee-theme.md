...
| テーマ | フロントマター | タイトルスライド | セクション区切り |
|--------|---------------|-----------------|-----------------|
| border等 | `theme: border` | `<!-- _paginate: skip -->` | `<!-- _backgroundColor: #303030 --><!-- _color: white -->` |
| speee | `theme: speee` | `<!-- _class: lead --><!-- _paginate: skip -->` | `<!-- _class: lead -->` |
...
- **テーマを選んでからスライドを生成すること**: テーマ別のシステムプロンプトはスライド生成時に適用されるため、プレビューでテーマを切り替えるだけでは `_class: lead` 等のマークダウン構造は変わらない
- テーマ変更後にスライドを修正したい場合は、チャットで「セクション区切りをleadスタイルに直して」等と修正依頼する
