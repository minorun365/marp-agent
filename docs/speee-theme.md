...
| 特殊クラス | `.lead`（表紙・セクション区切り用、中央揃え）、`.end`（裏表紙用） |
...
- **通常スライド**: 白背景 + 紺色テキスト + 上部グラデーションバー
  - h1: font-weight: 900（太字）
  - h2: font-weight: bold（太字）
- **Leadスライド**: 紺→水色グラデーション背景（`linear-gradient(115deg, #080058, #008bcd)`） + 白テキスト + 中央揃え
  - h1: font-weight: 900（太字、タイトル用）
  - h2-h6: font-weight: normal（サブタイトルは太字でない）
  - CSS変数で `--color-foreground: white` に切り替え
- **Endスライド**: Leadと同じデザイン（裏表紙用、ページ番号非表示）
...
| タイトルスライドにlead適用 | OK | 紺→水色グラデーション背景＋白テキスト＋中央揃え |
...
| Beam | 白 | 黒 | 学会風、コーナーバー |
| **Speee** | **白（通常）/ グラデーション（lead）** | **紺（通常）/ 白（lead）** | **デフォルトテーマ、ビジネス向け** |
