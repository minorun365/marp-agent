...
現在の Amplify Gen 2 環境をそのまま別アカウントへ複製せず、パワポ作るマンを新しい AWS アカウントへ再構築する。既存の移行先環境には手を加えず、新環境を並行して作成してから切り替える。
...
| デプロイ | CDKD を開発・本番の両方で使用する。導入時の正確なバージョンを固定する |
...
| 認証 | Cognito User Pools。Google、メール OTP、パスキーに対応する |
...
flowchart LR
    ...
    AC --> ART["生成物と共有スライド<br>S3 + CloudFront"]
...
| 利用者 | 初回 | 2回目以降 |
|---|---|---|
| 既存ユーザー | 従来と同じメールアドレスとパスワードでログインし、User Migration Trigger で移行する | パスキー、Google、または移行済みのパスワード |
| Google を使う新規ユーザー | Google で登録とログインを同時に完了する | Google |
| メールを使う新規ユーザー | メール OTP で本人確認し、続けてパスキーを登録する | パスキー。復旧時はメール OTP |

メール OTP やパスキーのログインでは User Migration Trigger が起動しない。したがって、既存ユーザーにメールコードを強制せず、移行期間中だけ従来のパスワード入力を残す。これが既存ユーザーの操作を増やさず、旧パスワードも引き継げる経路になる。
...
ログインと新規登録をタブで分けず、最初の画面には「パスキーで続ける」「Google で続ける」「メールで続ける」の3つだけを表示する。既存ユーザー向けの旧パスワード移行は、初期画面下部の補助導線から専用画面へ移す。認証方式を同じ強さのカードで並べず、利用頻度と移行期だけ必要な操作の違いを視覚的に分ける。

メール OTP は同じ画面内で確認コード入力へ切り替え、パスキー登録は本人確認が終わった後に案内する。PC はブランド面と認証面の左右分割、スマートフォンはブランド面を短くしてフォームを主役にする。画面案は [ログイン画面デザインモック](./mocks/login-design.html) で確認できる。
...
PDF や PowerPoint の大きなファイルは、最終的に SSE の Base64 へ載せず、S3 の短時間だけ有効なダウンロード URL で渡す。共有スライドの公開 URL は既存機能を維持する。
...
| スタック | 主なリソース | 更新頻度 |
|---|---|---|
| Foundation | KMS、S3、Secrets Manager、ログ、予算通知 | 低い |
| Auth | Cognito User Pool、App Client、Google、移行 Lambda | 低い |
| Agent | ECR、AgentCore Runtime、Memory、評価、実行ロール | 高い |
| Web | Web Lambda、Function URL、CloudFront、証明書、DNS | 高い |
| Sharing | 共有用 S3、CloudFront、証明書、DNS | 中程度 |
...
2. 初回導入時と更新時に、対象リソースとプロパティが CDKD で対応済みか確認する。
3. `synth`、対象スタックの `diff`、`deploy --dry-run` を通してからデプロイする。
4. 本番では明示したスタックだけを `--full-wait` でデプロイし、CloudFront が配信可能になるまで待つ。
5. デプロイ後に `state show`、`events`、ドリフト検査、スモークテストを行う。
6. 状態バケットは暗号化、バージョニング、公開拒否を有効にし、別の保全先へ定期コピーする。
7. 前のコンテナイメージと Web 配信物を残し、Runtime の版戻しと `cdkd rollback` の両方を手順化する。
8. CDKD で対応できないリソースが出た場合は回避フラグで通さず、CloudFormation へ戻すかスタックを分離する。

新規アカウントのため、既存の CloudFormation スタックを CDKD へ暗黙に移管する作業は発生しない。既存の Amplify 環境は切り替え完了まで別管理のまま残す。

## ローカル開発

ローカルへ AWS を丸ごと再現しない。CDKD のローカル実行と、開発用 AWS リソースを組み合わせる。

| 用途 | 起動内容 | AWS との接続 |
|---|---|---|
| 画面だけを素早く直す | Vite と既存モック | なし |
| 普段の機能開発 | Vite、`cdkd local start-agentcore --watch` | Bedrock、Secrets Manager、開発用 Cognito |
| 配信経路まで確認 | React をビルドし、`cdkd local start-cloudfront` と `start-agentcore` を起動 | 開発用 Cognito と必要な AWS サービス |
| パスキーと Google の確認 | 開発用の公開 URL にデプロイ | 開発用 Cognito、DNS、HTTPS を含む |
...
```text
npm run dev:ui       # Vite + モック
npm run dev          # Vite + ローカルAgentCore + 開発用AWS認証
npm run dev:full     # CloudFront/Lambda Web Adapterを含む本番相当のローカル確認
npm run deploy:dev   # パスキー、Google、HTTPSを含む開発環境へ反映
```

パスキーはオリジンと Relying Party ID の組み合わせを検証する必要がある。localhost だけを合格条件にせず、専用の開発 URL で登録、ログイン、削除、復旧まで確認する。
...
秘密情報は含めない。Web Lambda が CDK の出力から動的に返すため、同じ React ビルド成果物を開発・本番で使える。ローカルでは `cdkd state show` の出力から同じ形式の一時ファイルを生成する。
...
| `deploy-dev` | テスト、ビルド、CDKD の差分確認、開発環境へのデプロイ、動作確認を行う |
...
|---|---|
| ブランチごとの環境 | `dev` と `prod` の固定環境。必要時だけ一時スタックを作る |
...
| Sandbox の起動 | CDKD のローカル実行と開発用 AWS 環境 |
| デプロイ履歴と復旧 | CDKD の状態・イベント・ドリフト、ECR の版、復旧スキル |
| プレビュー環境 | 原則廃止。大きな変更だけ一時スタックを作る |

追加で見落としやすい項目は次のとおり。

- 新しい AWS アカウントで Bedrock、AgentCore、Lambda、SES のクォータとモデル利用可否を実測する。
- AWS Organizations のクレジット共有と、Kimi K2.5 の費用へクレジットが適用されるか請求画面で確認する。
- Google OAuth の許可済みリダイレクト URL を開発・本番の両方へ登録する。
- メール OTP の送信元ドメイン、DKIM、SES 本番利用申請、バウンス対応を用意する。
...
3. 開発環境とローカル起動を先に完成させる。
4. Cognito と User Migration Trigger を構築し、既存ユーザー、新規ユーザー、Google、メール OTP、パスキーを確認する。
...
2. Cognito の SDK ベースのパスキー、Google OAuth、既存ユーザー移行を同じ開発用 User Pool で通す。
