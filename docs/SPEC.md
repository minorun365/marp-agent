...
| スライド | 出力形式 | PDFのみ（MVP） |
| エージェント | 性格 | プロフェッショナル |
| エージェント | ツール | web_search（Tavily）, output_slide, generate_tweet_url |
...
| ツイート案作成中 | ステータスメッセージ + スピナー |
| ツイート案作成完了 | ステータスメッセージ + チェックマーク + ツイートリンク表示 |
| 完了 | プレビュータブに自動切り替え |
...
| スライド生成中 | 「スライドを作成中...（30秒ほどかかります）」 | スピナー（回転アニメーション） |
| スライド生成完了 | 「スライドを生成しました」 | チェックマーク（緑色✓） |
| ツイート案作成中 | 「ツイート案を作成中...」 | スピナー（回転アニメーション） |
| ツイート案作成完了 | 「ツイート案を作成しました」 | チェックマーク（緑色✓） |
...
| `generate_tweet_url` | ツイート投稿用のURLを生成。PDFダウンロード後にXでのシェアを提案する際に使用 |
