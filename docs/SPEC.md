...
| スライド生成中 | 「スライドを作成中...（20秒ほどかかります）」 | スピナー（回転アニメーション） |
| スライド生成完了 | 「スライドを作成しました」 | チェックマーク（緑色✓） |
| ツイート作成中 | 「ツイート案を作成中...」 | スピナー（回転アニメーション） |
| ツイート作成完了 | 「ツイート案を作成しました」 | チェックマーク（緑色✓） |

**重複防止**: 同じ「Web検索中...」ステータスが複数回表示されないよう、進行中のステータスがある場合はスキップする

**複数回検索時の挙動**: 2回目のweb_search開始時、テキスト受信により1回目の「Web検索中...」が自動的に「Web検索完了」に更新され、新しい「Web検索中...」ステータスが追加される
...
### 1. タイトルスライド（1枚目）【必須】
最初のスライドには**必ず** `top` クラスを使用。
### 2. 仕切りスライド（セクション区切り）【推奨】
3つ以上のセクションがある場合、`crosshead` クラスを使用。
### 3. 通常スライド
クラス指定なし。
### 4. 最後のスライド【必須】
**必ず** `end` クラスを使用。内容は空にする。

## スライド構成テクニック（必ず従うこと！）
単調な箇条書きの連続を避け、以下のテクニックを織り交ぜてプロフェッショナルなスライドを作成。

- **セクション区切りスライド【必須】**: 3〜4枚ごとに背景色を変えた中タイトルスライドを挟む
- **表（テーブル）**: 比較・一覧に最適
- **引用ブロック**: 重要なポイントや定義の強調に
- **==ハイライト==**: キーワードの強調に
- **参考文献スライド**: Web検索した場合は最後に出典スライドを追加（`tinytext`クラス）
