...
#### 見送り理由: キャッシュ可能な部分が小さすぎる

Bedrockプロンプトキャッシュ（Bedrock APIサーバー側）とAgentCoreセッション（コンテナメモリ内）は別物。
キャッシュが効くのは**全リクエスト共通のプレフィックス部分**のみ。

```
[System Prompt + Tools] → ~2,043 tokens ← キャッシュ対象（全体の12%）
[会話履歴]             → ユーザーごとに異なる ← キャッシュ不可
[ユーザーメッセージ]    → 毎回異なる ← キャッシュ不可
```

AgentCore Runtimeのセッションは**15分で非アクティブ終了**するため、会話履歴はリセットされる。
つまりキャッシュ可能なのは平均入力 ~16,849 tokens のうち **2,043 tokens（12%）だけ**。

#### 損益計算

```
■ キャッシュRead時の節約（1リクエストあたり）
  通常入力:  2,043 × $3.00/MTok = $0.0061
  CacheRead: 2,043 × $0.30/MTok = $0.0006
  → 節約: $0.0055/リクエスト

■ CacheWrite コスト増（5分→1時間）
  5分TTL:  2,043 × $3.75/MTok = $0.0077/回
  1時間TTL: 2,043 × $6.00/MTok = $0.0123/回
  → 追加コスト: +$0.0046/回（+60%）

■ 45リクエスト/日でヒット率100%でも
  純粋な効果: ~$0.25/日（月$7程度）
```

施策2（window_size=6）による -63% 削減と比較して、**費用対効果が2桁違う**ため見送り。
...
| 順番 | 施策 | 工数 | 期待効果 | 状態 |
|------|------|------|----------|------|
| 1 | **施策2: 会話履歴トリミング** | 小 | 大 | ✅ 実装済み（window_size=6） |
| 2 | **施策5: System Prompt圧縮** | 小 | 中 | ✅ 実装済み |
| 3 | **施策3: Markdown二重送信の解消** | 小 | 中 | ✅ 実装済み |
| 4 | **施策6: ユーザー利用制限** | 中 | 大 | 未着手 |
| 5 | **施策4: キャッシュヒット率向上** | 中 | ~~大~~ ほぼなし | 見送り（キャッシュ対象が入力の12%のみ） |
| 6 | **施策1: Web検索サブエージェント** | 中 | - | 断念（品質低下が不可避） |

> **方針**: 工数「小」の施策（2, 5, 3）を先に片付けて即効性のあるコスト削減を実現。
> 施策2が圧倒的に効果大（セッション単価 -63%）。施策4は分析の結果、効果なしと判明し見送り。
>
...
| 2026-02-12 | 施策4: キャッシュ可能部分が入力の12%（2,043tok）のみで効果$0.25/日と判明、見送り |
...
| ~~施策4: キャッシュヒット率向上~~ | ~~-$5/日~~ | 見送り（効果なし） |
| ~~施策1: Web検索サブエージェント~~ | ~~-$4/日~~ | 断念（品質低下） |

### 実績ベース（2026-02-12 時点）

| 指標 | 施策前 (2/5-2/9) | 施策後 (2/11, window_size=6) | 改善 |
|------|-----------------|----------------------------|------|
| セッション単価 | $0.58/回 | $0.21/回 | **-63.4%** |
| 日額コスト（main） | $30.52/日 | $4.49/日 | -85% |
| 月間推定（同セッション数換算） | ~$915/月 | ~$330/月 | **-64%** |

> 施策2（会話履歴トリミング、特にwindow_size=6への調整）が圧倒的に効果大。
> 施策4は分析の結果キャッシュ対象が入力の12%のみで効果$0.25/日と判明し見送り。
> 施策6（利用制限）は未実装で、追加で削減可能。
...
### ~~Web検索パラメータの変更~~ → cost_opt_v2 で実施済み

- 当初はスライド品質低下リスクを懸念してスコープ外としていた
- 施策1（サブエージェント）断念後、Tavilyクレジット逼迫を受けてパラメータ変更を実施
- 詳細は「Tavily最適化 効果検証」セクションを参照
...
| ~~施策4: キャッシュ1時間化~~ | 見送り | 効果なし（$0.25/日） | - |
| ~~施策1: Web検索サブエージェント~~ | 断念 | 品質低下が不可避 | - |
...
## Tavily最適化 効果検証（2026-02-12 CloudWatch実測）

### 変更内容（cost_opt_v2）

- **コミット日時**: 2026-02-10 20:01 JST
- **変更ファイル**: `amplify/agent/runtime/tools/web_search.py`
- `search_depth`: advanced → basic（2クレジット→1クレジット/検索）
- `max_results`: 5 → 3

### Web検索回数の日別推移

| 日付 (UTC) | web_search回数 | クレジット単価 | 推定クレジット消費 |
|:--|--:|--:|--:|
| 2/5 (水) | 47 | 2 (advanced) | 94 |
| 2/6 (木) | **256** | 2 | **512** |
| 2/7 (金) | 148 | 2 | 296 |
| 2/8 (土) | 117 | 2 | 234 |
| 2/9 (日) | 114 | 2 | 228 |
| **2/10 前半** | 30 | 2 | 60 |
| **2/10 後半** | 63 | **1 (basic)** | **63** |
| **2/11 (火)** | 53 | 1 | **53** |
| 2/12 (水) | 34 | 1 | 34 (途中) |

> 2/10のデプロイ境界: UTC 11:01（JST 20:01）

### 検索結果のサイズ変化

| 期間 | 検索結果の平均サイズ | 変化 |
|------|-------------------|------|
| 最適化前 (2/5-2/9) | ~26,163 文字 | - |
| 2/10 デプロイ後 | ~19,881 文字 | -24% |
| 最適化後 (2/11) | ~7,958 文字 | **-70%** |

`max_results` 5→3（件数40%減）+ `search_depth` advanced→basic（各結果のコンテンツ量も減少）の複合効果。
検索結果サイズの削減はBedrock側の入力トークンコスト削減にも寄与する。

### クレジット消費の比較

| 指標 | 最適化前 (2/5-2/9) | 最適化後 (2/11) | 変化 |
|------|-------------------|----------------|------|
| 日平均クレジット | ~273/日 | ~53/日 | **-81%** |
| 月間推定 | ~8,190/月 | ~1,590/月 | **-81%** |

削減の内訳:
1. **クレジット単価半減**: advanced(2) → basic(1) = -50%
2. **利用量の自然減**: 2/6のピーク(256回)のような突発的大量利用が施策後は発生していない

### 残りクレジットの見通し（2026-02-12時点）

| キー | 使用量 | 上限 | 残り | 状態 |
|------|--------|------|------|------|
| キー1 | 1,006 | 1,000 | 0 | 枯渇（超過） |
| キー2 | 1,000 | 1,000 | 0 | 枯渇 |
| キー3 | 839 | 1,000 | 161 | 使用中 |
| キー4 | 0 | 1,000 | 1,000 | 未使用 |
| キー5 | 0 | 1,000 | 1,000 | 未使用 |
| **合計** | **2,845** | **5,000** | **2,161** | |
...
- 最適化後の観測期間が短い（実質2/11の1日分）。2-3日追加観測で精度向上が望ましい
- 2/6のような突発的ピーク（256回/日）が再発するとペースは上振れする
- 検索回数自体はモデルの判断に依存するため、search_depth/max_resultsの変更とは独立して変動する
