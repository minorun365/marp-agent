...
> **注意**: TODO管理は **mainブランチのみ** で行います。派生版側のTODOファイルは参照用のリンクのみです。
...
| # | タスク | 工数 | 状態 | ラベル | main 実装 | main docs | variant 実装 | variant docs |
|---|--------|------|------|--------|-----------|-----------|----------|----------|
...
**概要**: 企業ブランド向けのスライドテンプレートを新規作成する。

**修正方法**: 既存テンプレートを参考にブランド用のMarpテーマCSS・画像を作成し、テンプレート選択UIに追加する。
