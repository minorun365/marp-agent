...
1. **型をローカルで定義**（シンプル、2-3箇所でしか使わない場合に推奨）
   ```typescript
   // types.ts 内で直接定義
   type ModelType = 'sonnet' | 'sonnet5' | 'kimi' | 'glm' | 'opus' | 'sol' | 'grok';  // MODEL_OPTIONS で実際に有効な選択肢を管理
   ```
...
### ファイルアップロードUI（ChatInput）

参考資料（PDF）を添付するための📎ボタンとファイルチップを実装。
...
- 📎ボタンは入力欄の右端にさりげなく配置（`text-gray-300`、ホバーで `text-gray-500`）
- ファイル選択中は📎アイコンがアクセントカラー（`text-[#5ba4d9]`）に変化
- ファイルチップは入力欄の上に表示（📄絵文字 + ファイル名 + サイズ + ✕ボタン）
- テキストなし + ファイルありでも送信可能（デフォルトプロンプト使用）

#### バリデーション（フロントエンド側）

| チェック | 制限値 | 方法 |
|---------|--------|------|
| ファイル形式 | PDF のみ | `accept=".pdf"` + JS検証 |
| ファイルサイズ | 10MB | `file.size > MAX_FILE_SIZE` |

#### Base64変換（useChatMessages.ts）

`FileReader.readAsDataURL` で変換後、data URLプレフィックスを除去：
