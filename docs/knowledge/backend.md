...
# Sonnet 5 / GLM-5 / Opus 4.6（設定保持・現在無効）
...
|--------|-------------------|--------------|-------------|------|
| Claude Sonnet 4.6 | AIP経由 | `"default"` | `"default"` | デフォルト |
| Kimi K2.5 | なし | `None` | `None` | 高速オプション |
| Claude Sonnet 5 | AIP経由 | `"default"` | `"default"` | 設定保持・現在無効 |
| GLM-5 | なし | `None` | `None` | 設定保持・現在無効 |
| Claude Opus 4.6 | AIP経由 | `"default"` | `"default"` | 設定保持・現在無効 |

Sonnet 4.6をデフォルトとし、Kimi K2.5を選択可能にしている。無効なモデルがAPIへ指定された場合はSonnetへ正規化する。無効モデルを再検証するときは、`types.ts`の`MODEL_OPTIONS`と`config.py`の`ENABLED_MODEL_TYPES`を同時に変更する。
...
Kimi K2.5には共通プロンプトへ追加ルールを連結する。指定総枚数、中タイトル数、文章量、太字、見出し階層、根拠のない数値、同一表現の連続を制約し、`output_slide`直前に自己検査させる。Sonnet 4.6には追加しない。
...
- **tool_use重複はフロントエンド側で吸収**: LLMのストリーミングでは同一ツールの `current_tool_use` イベントがチャンクごとに発生する。バックエンドで重複排除すると、入力が完成する前のチャンクだけを採用する危険がある。フロントエンドではステータスを重複追加せず、`output_slide`のTipsタイマーも開始済みなら再起動しない。最初のイベントから3秒後にTipsを表示し、以降5秒ごとに切り替える
