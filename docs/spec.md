...
**豆知識ローテーション**: 最初の`output_slide`イベントから3秒後に豆知識を表示し、以降5秒ごとに切り替える。Kimi K2.5は同じツールイベントを複数回送ることがあるため、開始済みのタイマーは再起動しない。豆知識は「Tips: 〇〇」形式でフェードイン表示する。

**重複防止**: 同じステータスが複数回表示されないよう、進行中のステータスがある場合はスキップする

### モデル選択

試験運用中はKimi K2.5のみを有効にする。選択肢は「標準（Kimi K2.5）」を初期選択し、「高品質（Claude Sonnet 4.6） ※資金不足により停止中」は表示を残したまま選択不可にする。Sonnet 4.6、GPT-5.6 Sol、Sonnet 5、GLM-5、Opus 4.6の設定は再開に備えて保持するが、APIへ直接指定されてもKimi K2.5へ正規化する。会話開始後は履歴とモデルの不整合を避けるため、モデル選択全体を変更不可にする。
...
共通プロンプトは全モデルで共有する。試験運用中のKimi K2.5には、短いキーワードでも対象読者・目的・構成・デザイン・枚数を聞き返さず、必要なWeb検索から`output_slide`まで同じ応答内で完了する自律実行ルールを追加する。枚数未指定時は原則8枚、最大10枚とする。Kimiにはさらに、指定枚数の厳守、中タイトル数、文章量、太字、見出し階層、根拠のない数値、同一表現の連続を制御する追加ルールを連結する。Sonnet 4.6とGPT-5.6 Sol用のプロンプトは再有効化に備えて保持する。
...
- `model_type`: `"sonnet"`（デフォルト）、`"sol"`、`"kimi"`
...
- [x] モデル表示/選択基盤（標準のKimi K2.5のみ有効。Sonnet 4.6は停止理由を表示して選択不可、その他のモデルも設定保持・無効）
- [ ] 画像アップロード・挿入
- [ ] スライド履歴管理
