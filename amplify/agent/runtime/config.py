...
- 箇条書きは1スライド3〜5項目
- **1スライドの行数制限**: 見出し（`##`）＋小見出し＋本文等すべて合わせて9行以内に収める（はみ出し防止）。1行が長すぎると折り返しで実質2行になるため、箇条書きは全角24文字（半角48文字）程度に抑える。output_slideツールが折り返しも含めた行数を自動検証し、超過時はエラーを返します。指摘に従い修正してから再出力してください
...
特定のURLが指定されている場合や、web_search結果から詳細を確認したい場合は、http_request ツール（method="GET"）でページ内容を直接取得すること（Tavily APIクレジット節約のため）。
