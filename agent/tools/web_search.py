...
# （2026-08-22、KAG版のリアーキ後に実際に発生）。実装側の上限も期待値に寄せる。
MAX_SEARCH_CALLS = 3
...
def web_search(query: str) -> str:
    ...
    - **多くの依頼は1〜2回で足りる。1回目の結果で書けるなら、それ以上調べない。**どうしても
      埋まらない論点があるときだけ足し、3回を超えない（1回に10秒前後かかるので回数がそのまま
      待ち時間になり、検索APIの無料枠も1回ずつ消費する）。同じ対象を言い換えて調べ直さない
    - 検索結果が依頼とかみ合わない（略語が別の製品を指している、同名の別人が出てくる等）ときだけ、
      対象を絞る語を足して調べ直してよい
    ...
    if _search_call_count >= call_limit:
        ...
        return (
            f"Web検索は上限{call_limit}回に達しました。"
            "これまでの検索結果だけを使ってスライドを作成してください。"
        )
